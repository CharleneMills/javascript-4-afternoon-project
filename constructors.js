...
function Employee(name, email, hireDate) {
  ...
  */
  ...
  User.prototype.removeSavedPost = function(postId){
    const myIndex = this.savedPosts.findIndex( post => post.id === postId);

    this.savedPosts.splice(myIndex, 1);
 }
  ...
  User.prototype.changePostRating = function(postId, newRating){
    ...
    this.savedPosts[myIndex].rating = newRating;
 }
